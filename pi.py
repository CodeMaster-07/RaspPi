...
def parse_battery_data(data):
    state = data[4]
    soc = data[5]
    total_voltage = (data[7] << 8 | data[6]) * 0.01  # 전체 전압은 0.01V 단위
...
def call_mysql_procedure(data, raspberry_pi, usb_port, ip_address, mac_address, rack_number):
    try:
...
if __name__ == '__main__':
    raspberry_pi = 'PI1'  # Replace with actual Raspberry Pi identifier
    ip_address = get_ip_address()
    mac_address = get_mac_address()
    rack_number = 1  # Replace with actual rack number if needed
        ...
        for usb_port in range(1, 11):
            ...
            try:
                ...
                if data:
                    ...
                    call_mysql_procedure(parsed_data, raspberry_pi, f'USB{usb_port}', ip_address, mac_address, rack_number)
                else:
